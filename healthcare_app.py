...
# --- UNIVERSAL HIGH-CONTRAST CSS FIX ---
st.markdown("""
<style>
    ...
    .stTextInput label, .stSelectbox label, .stSlider label, [data-testid="stWidgetLabel"] p, [data-testid="stWidgetLabel"] span {
        color: #ffffff !important;
...
with top_col1:
    st.markdown("### 🏥 ST. JUDE HOSPITAL OPERATIONS")

with top_col2:
    patient_search = st.text_input("Patient Lookup", placeholder="Search MRN...", label_visibility="collapsed")

with top_col3:
    if st.button("📋 Export Shift Report", use_container_width=True):
        st.session_state.report_generated = True
        st.success("Shift report compiled!")
...
with m1:
    ...
    <div class="card" style="border-left: 4px solid #48cae4;">
        ...
        <div style="color: #90e0ef; font-size: 11px; margin-top: 5px;">42 BEDS AVAILABLE SYSTEM-WIDE</div>
    </div>
    """, unsafe_allow_html=True)
...
with m2:
    ...
    <div class="card" style="border-left: 4px solid #00b4d8;">
        ...
        <div style="color: #90e0ef; font-size: 11px; margin-top: 5px;">-4 MINS VS PREVIOUS HOUR</div>
    </div>
    """, unsafe_allow_html=True)
...
with m4:
    ...
    <div class="card" style="border-left: 4px solid #ade8f4;">
        ...
        <div style="color: #90e0ef; font-size: 11px; margin-top: 5px;">BASED ON 310 EXIT SURVEYS</div>
    </div>
    """, unsafe_allow_html=True)
...
with col_c1:
    ...
    <div class="card">
        ...
        <span style='color: #ffffff; font-size: 12px; font-weight: 600;'>Emergency Department (92% Capacity)</span>
    </div>
    """, unsafe_allow_html=True)
    st.progress(0.92)
    
    st.markdown("<span style='color: #ffffff; font-size: 12px; font-weight: 600;'>Intensive Care Unit - ICU (85% Capacity)</span>", unsafe_allow_html=True)
    st.progress(0.85)
    
    st.markdown("<span style='color: #ffffff; font-size: 12px; font-weight: 600;'>Surgery & Recovery Wards (78% Capacity)</span>", unsafe_allow_html=True)
    st.progress(0.78)
...
with col_c2:
    ...
    fig_influx.update_layout(paper_bgcolor='#1c2541', plot_bgcolor='#1c2541', margin=dict(t=5, b=5, l=5, r=5), height=115, font=dict(color="#ffffff"), xaxis=dict(visible=False), yaxis=dict(visible=False))
...
with g1:
    ...
    fig_donut.update_layout(paper_bgcolor='#1c2541', plot_bgcolor='#1c2541', margin=dict(t=10, b=10, l=10, r=10), height=180, font=dict(color="#ffffff"), legend=dict(orientation="h", y=-0.25, font=dict(color="#ffffff", size=9)))
...
with g2:
    ...
    fig_gauge.update_layout(paper_bgcolor='#1c2541', plot_bgcolor='#1c2541', margin=dict(t=25, b=10, l=10, r=10), height=155, font=dict(color="#ffffff"))
...
with g3:
    ...
    fig_box.update_layout(paper_bgcolor='#1c2541', plot_bgcolor='#1c2541', margin=dict(t=5, b=5, l=5, r=5), height=170, font=dict(color="#ffffff"), showlegend=False, xaxis=dict(title=None), yaxis=dict(title=None))
